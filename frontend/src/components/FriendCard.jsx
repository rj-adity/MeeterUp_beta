...
const FriendCard = ({ friend }) => {
  ...
  // Notifications
  const friendNotifications = notifications.filter(n => n.sender === friend._id);
  ...
  return (
    <div className="card bg-base-200 hover:shadow-md transition-shadow relative">
      <div className="card-body p-4">
        ...
        <Link
          to={`/chat/${friend._id}`}
          className="btn btn-outline btn-sm w-full mb-3 flex justify-center"
        >
          Message
        </Link>
          ...
          <div className="grid grid-cols-3 gap-2 mb-2">
            ...
            <button
              ...
              disabled={
                ...
                pinnedFriends.length >= MAX_PINNED_FRIENDS
              }
            >
            ...
            {/* Friend bio */}
            <div className="col-span-3 text-sm text-gray-600 italic">{friend.bio}</div>
          </div>
        )}
      </div>
    </div>
